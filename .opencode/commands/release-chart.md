...
if [ ! -d "$CHART_PATH" ]; then
    echo "❌ Error: Chart '$CHART_NAME' not found in charts/ directory"
    ls -la charts/ 2>/dev/null || echo "📁 charts/ directory not found"
    exit 1
fi
...
if [ -z "$CURRENT_VERSION" ]; then
    echo "❌ Error: Could not read current version from Chart.yaml"
    exit 1
fi
...
if [ -z "$VERSION_BUMP" ]; then
    ...
    echo "  1) patch   - Backward-compatible bug fixes (0.1.8 → 0.1.9)"
    echo "  2) minor   - Backward-compatible new features (0.1.8 → 0.2.0)"
    echo "  3) major   - Breaking changes (0.1.8 → 1.0.0)"
    echo
...
if [ -n "$UPSTREAM" ]; then
    ...
    echo "⚠️  No upstream configured for branch $CURRENT_BRANCH"
    echo "💡 To set upstream, run: git push --set-upstream origin $CURRENT_BRANCH"
fi
...
if [ "$CURRENT_BRANCH" = "main" ]; then
    ...
    echo "💡 Remember to merge this release to main branch when ready"
fi